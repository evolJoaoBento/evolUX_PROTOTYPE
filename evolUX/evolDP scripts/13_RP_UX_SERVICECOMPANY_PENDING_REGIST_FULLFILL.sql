IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[RP_UX_SERVICECOMPANY_PENDING_REGIST_FULLFILL]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[RP_UX_SERVICECOMPANY_PENDING_REGIST_FULLFILL] AS' 
END
GO

ALTER  PROCEDURE [dbo].[RP_UX_SERVICECOMPANY_PENDING_REGIST_FULLFILL]
	@RunID int,
...
BEGIN
	...
	ON ISNULL(r.Closed,0) = 0 AND r.RunID = @RunID 
		...
		AND NOT EXISTS (SELECT TOP 1 1 
				...
				WHERE RunID = f.RunID 
					AND FileID = f.FileID
					AND ErrorID = 0
					AND RunStateID = @FullFillStateID
	...
	ON 	fl1.RunID = fl2.RunID
		...
		AND fl2.runstateid = @PrintedStateID
		AND fl2.ErrorID = 0
