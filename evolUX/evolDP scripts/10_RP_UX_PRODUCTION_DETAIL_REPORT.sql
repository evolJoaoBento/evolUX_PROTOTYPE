IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[RP_UX_PRODUCTION_DETAIL_REPORT]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[RP_UX_PRODUCTION_DETAIL_REPORT] AS' 
END
GO
ALTER  PROCEDURE [dbo].[RP_UX_PRODUCTION_DETAIL_REPORT] 
