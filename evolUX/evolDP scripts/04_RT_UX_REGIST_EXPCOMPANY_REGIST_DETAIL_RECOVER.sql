IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[RT_UX_REGIST_EXPCOMPANY_REGIST_DETAIL_RECOVER]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[RT_UX_REGIST_EXPCOMPANY_REGIST_DETAIL_RECOVER] AS' 
END
GO
ALTER  PROCEDURE [dbo].[RT_UX_REGIST_EXPCOMPANY_REGIST_DETAIL_RECOVER]
...
AS
	...
	BEGIN
		...
		IF NOT EXISTS(SELECT TOP 1 1
				FROM RT_POSTAL_OBJECT po WITH(NOLOCK),
					RT_EXPCOMPANY_REGIST_DETAIL_FILE erf WITH(NOLOCK)
				WHERE po.PostObjID = @EndPostObjID
				AND po.PostObjRunID = @RunID
				AND po.PostObjRunID = erf.RunID
	...
	INSERT INTO RT_EXPEDITION_REGIST_DETAIL_RECOVER_REGIST(RunID, FileID, RequestID, StartPostObjID, StartSeqNum, EndPostObjID, EndSeqNum, Reason, UserName, RegistrationTimeStamp, RecNumber, RecStartTime, RecEndTime)
	SELECT @RunID, @FileID, ISNULL(MAX(RequestID),0)+1, @StartPostObjID, @StartSeqNum, @EndPostObjID, @EndSeqNum, NULL, @UserName, CURRENT_TIMESTAMP, NULL, NULL, NULL
	FROM RT_EXPEDITION_REGIST_DETAIL_RECOVER_REGIST
	WHERE RunID = @RunID AND FileID = @FileID
	IF (@@Error<>0)
	BEGIN
		ROLLBACK TRANSACTION
